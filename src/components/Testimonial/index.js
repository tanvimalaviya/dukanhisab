...
//           <button
//             onClick={prevSlide}
//             className="bg-white text-logotype p-3 rounded-full shadow-lg 
...
const Testimonials = () => {
  ...
  return (
    <section className="bg-logotype text-white py-16 relative overflow-hidden">
      <div className="container mx-auto px-6 text-center">
        {/* Heading */}
        <h2 className="text-4xl font-bold mb-4">Testimonials</h2>
        <p className="mb-12 max-w-2xl mx-auto text-gray-200">
        ...
        <div className="flex justify-center gap-8 flex-wrap relative">
          <AnimatePresence mode="wait">
            {Array.from({ length: cardsToShow }).map((_, index) => {
              const item =
                testimonials[(current + index) % testimonials.length];
              return (
                <motion.div
                  key={item.id}
                  initial={{ opacity: 0 }}
